...
top_img: >-
  http://121.36.193.119/api/file/getBlogImage?imagePath=assets/articleCover/ByteDance.png
cover: >-
  http://121.36.193.119/api/file/getBlogImage?imagePath=assets/articleCover/ByteDance.png
tags:
  - 性能优化
...
![](http://121.36.193.119/api/file/getBlogImage?imagePath=assets/articleSource/2024-10-18-如何优雅地重试/img.png)

<h3 id="0c4f3d55"><font style="color:black;">重试的风险</font></h3>
...
![](http://121.36.193.119/api/file/getBlogImage?imagePath=assets/articleSource/2024-10-18-如何优雅地重试/img_1.png)

<font style="color:black;">更可怕的是，重试还会存在链路放大的效应，结合下图说明一下：</font>

![](http://121.36.193.119/api/file/getBlogImage?imagePath=assets/articleSource/2024-10-18-如何优雅地重试/img_2.png)
...
<font style="color:black;">基于以上的背景，为了让业务方能够灵活安全的使用重试，我们字节跳动直播中台团队设计和实现了一个重试治理组件，具有以下优点：</font>
...
![](http://121.36.193.119/api/file/getBlogImage?imagePath=assets/articleSource/2024-10-18-如何优雅地重试/img_3.png)
...
<font style="color:black;">对于一些暂时性的错误，如网络抖动等，可能立即重试还是会失败，通常等待一小会儿再重试的话成功率会较高，并且也可能打散上游重试的时间，较少因为同时都重试而导致的下游瞬间流量高峰。决定等待多久之后再重试的方法叫做退避策略，我们实现了常见的退避策略，如：</font>
...
![](http://121.36.193.119/api/file/getBlogImage?imagePath=assets/articleSource/2024-10-18-如何优雅地重试/img_4.png)
...
![](http://121.36.193.119/api/file/getBlogImage?imagePath=assets/articleSource/2024-10-18-如何优雅地重试/img_5.png)

<font style="color:black;">在链路中，推进每层都接入重试组件，这样每一层都可以通过识别这个标志位来停止重试，并逐层往上传递，上层也都停止重试，做到链路层面的防护，达到“只有最靠近错误发生的那一层才重试”的效果。</font>
...
![](http://121.36.193.119/api/file/getBlogImage?imagePath=assets/articleSource/2024-10-18-如何优雅地重试/img_6.png)
...
![](http://121.36.193.119/api/file/getBlogImage?imagePath=assets/articleSource/2024-10-18-如何优雅地重试/img_7.png)

<font style="color:black;">这种情况的本质原因是因为链路上的超时时间设置得不合理，上游和下游的超时时间设置的一样，甚至上游的超时时间比下游还要短。在实际情况中业务一般都没有专门配置过 RPC 的超时时间，所以可能上下游都是默认的超时，时长是一样的。为了应对这种情况，我们需要有一个机制来优化超时情况下的稳定性，并减少无用的重试。</font>
...
![](http://121.36.193.119/api/file/getBlogImage?imagePath=assets/articleSource/2024-10-18-如何优雅地重试/img_8.png)
...
![](http://121.36.193.119/api/file/getBlogImage?imagePath=assets/articleSource/2024-10-18-如何优雅地重试/img_9.png)
...
![](http://121.36.193.119/api/file/getBlogImage?imagePath=assets/articleSource/2024-10-18-如何优雅地重试/img_10.png)

<font style="color:black;">DDL 的方式能有效减少对下游的无效调用，我们在重试治理中也结合了 DDL 的数据，在每一次发起重试前都会判断 DDL 的剩余值是否还大于 0 ，如果已经不满足条件了，那也就没必要对下游重试，这样能做到最大限度的减少无用的重试。</font>
...
| **<font >策略</font>** | **<font >说明</font>** |
| --- | --- |
| <font>重试熔断</font> | <font >请求失败 / 成功 > 0.1 时停止重试</font> |
| <font>链路上传错误标志</font> | <font >下层重试失败后上传错误标志，上层不再重试</font> |
| <font >链路下传重试标志</font> | <font >重试请求特殊标记，下层对重试请求不会重试</font> |
| <font >DDL</font> | <font >当剩余时间不够时不再发起重试请求</font> |
| <font >框架熔断</font> | <font >微服务框架本身熔断、过载保护等机制也会影响重试效果</font> |
