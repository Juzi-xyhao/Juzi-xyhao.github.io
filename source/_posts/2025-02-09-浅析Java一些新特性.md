...
该特性在 JDK16 正式引入（JDK16 中一同引入的还有用于科学计算的 Vector API，见  
...
而且单论性能，预热后的JIT也不输native image。graalVM 的 native image为了提前编译做出了很多取舍，比如在以前收费的企业版里，G1要付费版才支持，而且吞吐量下降明显。  
目前来看 graalVM 的 native image最大的优点就是启动快，初始内存占用低。但不推荐长期运行的程序用它。
