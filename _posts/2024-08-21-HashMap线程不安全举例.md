...
假如15这个哈希桶是null的，A和B并发执行中都知道15是null
B先执行，15 = (2,"two")。
A后执行，15 = (1,"one")。
正常来说应该是15 = (2,"two")**, **15.next = (1,"one")
但是A把B的数据覆盖了。get(2)是get不到的
结合源码看看：
...
顺便吐槽一下源码的代码风格，在判断语句里给变量赋值，给我整懵了，直接看最后一个else的时候发现变量怎么定义了没赋值？仔细一看原来是在判断语句里赋值，太恶心了。就不能在定义时就赋值吗？
