...
以下三点问题均引用自公众号原文。
## j**ava引以为傲的“一次编译，处处运行”没用了**
以往依托于字节码和JVM，确实可以处处运行。但是静态编译只是把字节码作为中间变量，是生成二进制可执行文件的中间工具。为windows平台编译的exe可执行文件没办法在mac和linux上接着执行。

## **java 程序的许多动态特性都不直接生效了**
例如动态类加载，反射，动态代理等。

- 动态类加载失效：Native Image在构建时就需要知道应用会用到的所有类，然后把这些类打包进二进制文件。这样在运行时就不需要再加载任何类了，因为所有需要的类都已经在里面了。
- 反射失效：java的反射的原理是根据类的字节码获取类的信息。而在静态编译中，由于没有了字节码的概念，反射就失效了
- 动态代理失效：不管JDK代理还是CGLIB代理，都是基于原有字节码创建新的字节码。而在静态编译中，由于没有了字节码的概念，动态代理自然就和反射一样失效了。

## **基于字节码改写实现的 Java Agent 将不再适用**
...
# 如何解决静态编译带来的问题？
参考原文第二部分：![image.png](/assets/articleSource/2024-07-14-GraalVM/img.png)

![image.png](/assets/articleSource/2024-07-14-GraalVM/img_1.png)
![image.png](/assets/articleSource/2024-07-14-GraalVM/img_2.png)
![image.png](/assets/articleSource/2024-07-14-GraalVM/img_3.png)
